...
var message, x;  // var 声明（函数作用域、会提升，推荐改用 let/const）
    message = document.getElementById("p01");  // 按 id 查找 DOM 元素（id≈主键），返回该节点
    message.innerHTML = "";  // 设置元素内部 HTML 内容（会重新解析 HTML）
    x = document.getElementById("demo").value;  // 按 id 查找 DOM 元素（id≈主键），返回该节点
try {  // try 块：捕获可能出错的代码
    if(x == "") throw "值是空的";  // 抛出异常（≈ 后端的 throw）
    if(isNaN(x)) throw "值不是一个数字";  // 抛出异常（≈ 后端的 throw）
    x = Number(x);
...
document.getElementById("demo").value = "";  // 按 id 查找 DOM 元素（id≈主键），返回该节点
}
}
