/**
 * 章节: React 参考手册
 * 来源: 31-reference/example_14.jsx（形状C | hooks）
...
export const title = "React 参考手册 · example_14";
export const sourceFile = "31-reference/example_14.jsx";
export const shape = "C";
export const rawCode = "const initialState = { count: 0 };\n\nfunction reducer(state, action) {\n\n  switch (action.type) {\n\n    case 'increment':\n\n      return { count: state.count + 1 };\n\n    case 'decrement':\n\n      return { count: state.count - 1 };\n\n    default:\n\n      throw new Error();\n\n  }\n\n}\n\nfunction Counter() {\n\n  const [state, dispatch] = useReducer(reducer, initialState);\n\n  return (\n\n    <div>\n\n      <p>Count: {state.count}</p>\n\n      <button onClick={() => dispatch({ type: 'increment' })}>Increment</button>\n\n      <button onClick={() => dispatch({ type: 'decrement' })}>Decrement</button>\n\n    </div>\n\n  );\n\n}";
